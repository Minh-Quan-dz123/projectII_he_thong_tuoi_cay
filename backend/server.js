...
db.connect((err) => 
{
  if(err) 
  {
    console.error("❌ Lỗi kết nối zới MySQL:", err);
    return;
  }
...
// 4: Kết nối tới HiveMQ Cloud
const mqttClient=mqtt.connect("mqtts://8c8b67f172b549eba06b16f265f2f580.s1.eu.hivemq.cloud", {
...
mqttClient.on('connect',()=>{
  ...
  // đăng ký để nhận dữ liệu, nếu chỉ gửi thì dùng publish
    mqttClient.subscribe(['temperature_humidity','get_wateringLimited', 'get_watering_cycle'], (err) => { 
...
mqttClient.on('message',(topic,message)=>{
  if (topic==='temperature_humidity') // nếu là topic 2: nhận nhiệt độ, độ ẩm
  {
    try 
    {
      ...
      console.log('Dữ liệu cảm biến:', latestSensorData);// in ra để check
  ...
  { 
    try 
    {
      ...
      {
        console.log('✅ Đã nhận giá trị waterLimit từ HiveMQ:', waterLimitValue);
  ...
  else if(topic==='get_watering_cycle') // nếu là topic : lấy giá trị thời gian tưới cây
  { 
    try 
    {
      const watercycle=parseInt(message.toString()); // Chuyển dữ liệu thành số nguyên
      if(!isNaN(watercycle)&&watercycle>0) 
      {
        console.log('✅ Đã nhận giá trị thời gian tưới cây từ HiveMQ:', watercycle);
        io.emit('get_watering_cycle', watercycle); // cùng tên với hivemq
      } 
      else 
      {
        console.error('⚠️ Giá trị thời gian tưới cây không hợp lệ:', message.toString());
      }
    } 
    catch (err) 
    {
      console.error('⚠️ Dữ liệu thời gian tưới cây không hợp lệ:', message.toString());
    }
  }
});
...
io.on("connection", (socket)=>{
  ...
  // 5.4: Khi user nhập thời gian tưới cây
  socket.on("set_wateringtime", (cycleValue)=>{ // script.js : phần 4.1
    console.log("✅ Đã nhận được thời gian tưới cây từ web:", cycleValue);
    ...
    if (Number.isInteger(cycleValue)&&cycleValue>0) 
    {
      ...
      mqttClient.publish("set_watering_time",cycleValue.toString(),(err)=>{// gửi giá trị đó tới HiveMQ (dạng string)
        if (err) 
        {
          console.error("❌ lỗi gửi giá trị thời gian tưới cây tới HiveMQ:", err);
        } 
        else 
        {
        console.log(`✅ đã gửi giá trị thời gian tưới cây ${cycleValue} tới HiveMQ `);
        }
      });
    } 
    else 
  ...
  // 5.5: Khi user nhập Độ Chịu Khát
  ...
  //5.6: khi user thêm lịch tưới cây
  ...
  // 5.11: Khi user yêu cầu giá trị waterLimit
  ...
  // 5.12: Khi user yêu cầu giá trị thời gian tưới cây
  socket.on("request_watering_cycle",()=>{ // script.js 7.2

    // Gửi yêu cầu tới HiveMQ để lấy giá trị thời gian tưới cây trước đó
    mqttClient.publish('request_watering_cycle', 'GET',{qos:1},(err)=>{// topic8
      if (err) {
        console.error('❌ Lỗi khi gửi yêu cầu thời gian tưới cây tới HiveMQ:', err);
        socket.emit('get_watering_cycle', null); // Gửi null nếu có lỗi
      } 
      else 
      {
        console.log('✅ Đã gửi yêu cầu độ thời gian tưới cây từ HiveMQ');
      }
    });
  });
  

  // 5.13: Khi user xóa hết các lịch
  socket.on("delete_all_schedules",()=>{ // script.js 6.7.2
    // Gửi yêu cầu tới HiveMQ giá trị -1 
    mqttClient.publish("delete_scheduleMQ","-1");// topic 4, gửi dưới dạng string
      console.log(`✅ Đã gửi tới hivemq giá trị xóa all lịch: -1`);

    // xóa hết lịch trong table watering_schedule_mysql
    const query = "DELETE FROM watering_schedule_mysql";
    db.query(query, (err, result) => {
      if (err) {
        console.error("❌ Lỗi khi xóa tất cả lịch trong MySQL:", err);
        return;
      }
      console.log("✅ Đã xóa tất cả lịch trong MySQL. Số lịch đã xóa:", result.affectedRows);
    });
  });
...
server.listen(3323, () => {
  console.log("Server đang chạy tại http://localhost:3323");
});
