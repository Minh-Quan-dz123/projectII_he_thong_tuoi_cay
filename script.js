...
function LuuChuKy() {
  ...
  const InputChuky = document.getElementById("InputChuky").value;
  const cycleValue = parseInt(InputChuky);

  // Kiểm tra giá trị là số nguyên dương
  if (!isNaN(cycleValue) && cycleValue > 0) {
    DanhsachChuKy.push(cycleValue); // Thêm giá trị vào danh sách
    updateDanhsachChuKy(); // Cập nhật giao diện
    DongChuKy(); // Đóng modal
...
function updateDanhsachChuKy() {
  const DanhsachChuKyDiv = document.getElementById("DanhsachChuKy");
  DanhsachChuKyDiv.innerHTML = "";

  // Hiển thị danh sách thời gian tưới cây
  DanhsachChuKy.forEach((cycle, index) => {
    const cycleDiv = document.createElement("div");
    cycleDiv.innerHTML = `
      <span>${cycle}</span>
      <button onclick="XoaChuKy(${index})">x</button>
    `;
    DanhsachChuKyDiv.appendChild(cycleDiv);
  });
}

// 4.3 xóa thời gian tưới cây
function XoaChuKy(index) {
  // Xóa khỏi danh sách
  DanhsachChuKy.splice(index, 1);
  updateDanhsachChuKy();
}
